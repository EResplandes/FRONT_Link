...
export default {
    data() {
        return {
            ...
            form: ref({}),
            statusLms: ref(null),
            statusMateriais: ref(null),
            pedidos: ref(null),
            statusSelecionadoLm: ref(null),
            statusSelecionadoMaterial: ref(null),
    ...
    mounted: function () {
        ...
        // Metódo responsável por buscar todas status de uma lm
        this.lmService.listarStatusLm().then((data) => {
            this.statusLms = data.statusLm;
        });
    ...
    methods: {
        ...
        abrirModalMudancaStatusLm(dados) {
            this.lmSelecionada = dados.id;
            this.modalMudancaStatusLm = true;
        },

        abrirModalMudancaStatusMaterial(dados) {
            this.materialSelecionado = dados.id;
            this.modalMudancaStatusMaterial = true;
        },
        ...
        alterarStatusLm() {
            this.lmService.alterarStatusLm(this.lmSelecionada, this.statusSelecionadoLm).then((data) => {
                if (data.status == 200) {
                    this.buscarLms();
                    this.showSuccess(data.resposta);
                    this.modalMudancaStatusLm = false;
                } else {
                    this.showError('Ocorreu um erro, entre em contato com o Administrador!');
                }
            });
        ...
        alterarStatusMaterial() {
            this.lmService.alterarStatusMaterial(this.materialSelecionado, this.statusSelecionadoMaterial).then((data) => {
                if (data.status == 200) {
                    this.buscarLms();
                    this.showSuccess(data.resposta);
                    this.modalMudancaStatusMaterial = false;
                    this.modalVisualizarLm = false;
                } else {
                    this.showError('Ocorreu um erro, entre em contato com o Administrador!');
                }
        ...
        liberaMaterialAlmoxarife(data) {
            this.lmService.liberaMaterialAlmoxarife(data.id).then((data) => {
                if (data.status == 200) {
                    this.buscarLms();
                    this.showSuccess(data.resposta);
                    this.modalVisualizarLm = false;
        ...
        bloqueiaMaterialAlmoxarife(data) {
            this.lmService.bloqueiaMaterialAlmoxarife(data.id).then((data) => {
                if (data.status == 200) {
                    this.buscarLms();
                    this.showSuccess(data.resposta);
                    this.modalVisualizarLm = false;
...
<template>
    ...
    <div class="grid">
                    ...
                    <Column header="...">
                        <template #body="slotProps">
                            <div class="flex gap-2">
                                ...
                                <Button :disabled="slotProps.data.quantidade == slotProps.data.quantidade_entregue" icon="pi pi-box" @click.prevent="abrirModalFaturamentoItem(slotProps.data)" class="p-button-rounded p-button-success" />
                                <Button icon="pi pi-book" @click.prevent="abrirModalHistoricoLancamentos(slotProps.data)" class="p-button-rounded p-button-help" />
                                <Button icon="pi pi-comments" @click.prevent="abrirChatMaterial(slotProps.data)" class="p-button-rounded p-button-primary" />
                                <Button icon="pi pi-pencil" @click.prevent="abrirModalMudancaStatusMaterial(slotProps.data)" class="p-button-rounded p-button-warning" />
                                <Button v-if="slotProps.data.liberado_almoxarife == 0" icon="pi pi-lock" @click.prevent="liberaMaterialAlmoxarife(slotProps.data)" class="p-button-rounded p-button-secondary" />
                                <Button v-else icon="pi pi-lock-open" @click.prevent="bloqueiaMaterialAlmoxarife(slotProps.data)" class="p-button-rounded p-button-secondary" />
                            </div>
                        </template>
                    </Column>
                </DataTable>
        ...
        <!-- Modal edição de status de lm -->
        <Dialog v-model:visible="modalMudancaStatusLm" maximizable modal header="ALTERAR STATUS" :style="{ width: '60rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <div class="flex flex-column justify-content-center align-items-center gap-3">
                <div class="flex items-center gap-5">
                    <div v-for="statusLm in statusLms" :key="statusLm.id" class="flex items-center gap-2">
                        <RadioButton v-model="statusSelecionadoLm" :inputId="'status-' + statusLm.id" :value="statusLm.id" />
                        <label>{{ statusLm.status }}</label>
                    </div>
                </div>
                <Button @click.prevent="alterarStatusLm()" label="ALTERAR STATUS" class="mr-2 mt-3 p-button-info col-12" />
            </div>
        </Dialog>

        <!-- Modal edição de status de material -->
        <Dialog v-model:visible="modalMudancaStatusMaterial" maximizable modal header="ALTERAR STATUS" :style="{ width: '90rem' }" :breakpoints="{ '1199px': '75vw', '575px': '90vw' }">
            <div class="flex flex-column justify-content-center align-items-center gap-3">
                <div class="flex items-center gap-5">
                    <div v-for="statusMaterial in statusMateriais" :key="statusMaterial.id" class="flex items-center gap-2">
                        <RadioButton v-model="statusSelecionadoMaterial" :inputId="'status-' + statusMaterial.id" :value="statusMaterial.id" />
                        <label>{{ statusMaterial.status }}</label>
                    </div>
                </div>
                <Button @click.prevent="alterarStatusMaterial()" label="ALTERAR STATUS" class="mr-2 mt-3 p-button-info col-12" />
            </div>
        </Dialog>
        ...
        <div class="col-12">
            ...
            <DataTable :value="lms" showGridlines tableStyle="min-width: 50rem">
                ...
                <Column header="...">
                    <template #body="slotProps">
                        <div v-if="slotProps.data.status != 'Aguardando'" class="flex gap-2">
                            ...
                            <Button icon="pi pi-pencil" @click.prevent="abrirModalMudancaStatusLm(slotProps.data)" class="p-button-rounded p-button-warning" />
                        </div>
                        <div v-else class="flex gap-2">
                            <Button label="Iniciar" icon="pi pi-check" @click.prevent="iniciarProcessoCompraLm(slotProps.data)" class="p-button-rounded p-button-success" />
                        </div>
                    </template>
                </Column>
            </DataTable>
